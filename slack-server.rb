...
def slack_callback(slack_data)
  ...
  HTTParty.post(slack_data['response_url'], body: "{'response_type':'in_channel', 'text': '*Current Tracking Report*', 'attachments': [#{ return_attatchments[0..-1] }] }")
end
...
post "/report" do
  ...
  status 200
  slack_data = request.POST
  return "Getting Report data, #{slack_data['response_url']}"
end
