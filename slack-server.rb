...
def slack_callback(slack_data)
  ...
  HTTParty.post(slack_data['response_url'], body: "{'response_type':'in_channel', 'text': '*Current Tracking Report* \n Triggered by #{slack_data['user_name']}', 'attachments': [#{ return_attatchments[0..-1] }] }")
end
